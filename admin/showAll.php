...
$qry_all = "SELECT * FROM resep";
$qry_private = "SELECT * FROM resep WHERE is_private = 1";
$qry_live = "SELECT * FROM resep WHERE is_approved = 1";
$qry_pending = "SELECT * FROM resep WHERE is_private = 0 AND is_approved = 0";

if($pil === "0") {
    $data = mysqli_query($conn, $qry_all);
} elseif ($pil === "1") {
    $data = mysqli_query($conn, $qry_private);
} elseif ($pil === "2") {
    $data = mysqli_query($conn, $qry_live);
} elseif ($pil === "3") {
    $data = mysqli_query($conn, $qry_pending);
}
// var_dump($data);
